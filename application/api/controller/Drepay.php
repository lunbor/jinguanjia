...
class Drepay extends Apibase{
	...
	public function authent(){	
		...
		//dump($data);
		if(empty($user)) return rel(-2,'该用户不存在');
		$credit	=Db::name('creditCard')->where(array('credit_id'=>$data['credit_id'],'status'=>1))->find();			//信用卡信息
		if(!$credit){
			return rel(-1,'信用卡信息已经失效');
		}
		$data = [
			'paymer_name' => $user['realname'],//持卡人姓名
			'paymer_idcard' => $user['card'],//持卡人身份证
			'paymer_bank_no' => $credit['credit_code'],//持卡人银行卡号
			'paymer_phone' => $credit['phone'],//持卡人预留手机号
		];
	    $outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->authent($data);
		//如果鉴权成功，去建商户
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			//新建个体商户
			$rel2 = $this->enternet($user,$credit,$rel1['authent_no']);
			
			if(isset($rel2['Code']) && $rel2['Code'] == '10000' && isset($rel2['Resp_code']) && $rel2['Resp_code'] == '40000'){
				   $aisle=Db::name('payAisle')->where(['block_id'=>6])->find();		//查询当前默认使用的还款渠道
				   $rate=3;
				   $dfee=100;
			       //先创建进件数据
					$insert=[
						'block_id'		=> $aisle['block_id'],
						'credit_id'		=> $credit['credit_id'],
						'credit_code'	=> $credit['credit_code'],
						'user_id'		=> $user['id'],
						'status'		=> 1,						//默认该进件状态为不可用
						'rate'			=> $rate,
						'dfee'			=> $dfee,
						'agent_id'		=> $aisle['agent_id'],
						'keyname'		=> $aisle['keyname'],
						'mer_id'		=> $rel2['user_no'],
						'ctime'			=> time()
					];
					$registId=Db::name('creditRegist')->insertGetId($insert);
					return rel(1,'开通成功');
			}else{
				//dump($rel2);
				if(isset($rel2['Resp_msg'])){
					if(isset($rel2['user_no']) && $rel2['user_no']!=''){
						   $aisle=Db::name('payAisle')->where(['block_id'=>6])->find();		//查询当前默认使用的还款渠道
						   $rate=3;
						   $dfee=100;
						   //先创建进件数据
						...
						return rel(-1,$rel2['Resp_msg']);
					}
				}else{
					return rel(-1,$rel2['Msg']);
				}
			}
	...
	//新建个体商户
	private function enternet($user,$credit,$auth_order_no){	
		//$data=$this->_data;
		//$user=$this->_user;
		//list($m,$y)=explode('-',$credit['exp_date']);
		//$y = substr($y,-2);
		$shenfen = $this->get_shenfen($user['card']);
		$area= $this->gethometownByIdcard(substr($user['card'],0,6));
		$data = [
			'auth_order_no' => $auth_order_no,//鉴权订单号(鉴权接口返回)
			'user_name' => $user['realname'],//持卡人姓名
			'id_no' => $user['card'],//身份证号
			'card_no' => $credit['credit_code'],//银行卡号
			'phone' => $credit['phone'],//银行预留手机号
			'bank_name' => $credit['bank_name'],//银行卡开户行名称
			'bank_branch' => $credit['bank_name'],//储蓄卡:银行卡开户支行名称,信用卡:银行卡开户行名称
			'bank_code' => $credit['bank_id'],//联行号
			'bank_coding' => $credit['bank_no'],//银行缩写
			'province' => $shenfen,//省 (身份证)
			'city' => $area,//城市(身份证)
			'county' => $area,//区、县(身份证)
			'address' => $area,//详细地址
			//'validity' => $m.$y,//有效期、信用卡必传(mmyy格式)
			'validity' => $credit['exp_date'],//有效期、信用卡必传(mmyy格式)
			'cvv2' => $credit['safe_code'],//安全码、信用卡必传
			'bank_type' => 6,//银行卡类型 1表示储蓄卡6表示信用
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->enternet($data);
		return $rel1;
		//dump($rel1);
	}
	...
	public function jihuoPay($data){
		...
		if($credit_regist['qd_id']!=''){
			return true;
		}
		$data2 = [
			'user_no'=> $credit_regist['mer_id'],
			'smsCode'=> '12345',
			'entryCardOrderNo'=> '12345',
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->jihuoPay($data2);
		//dump($rel1);
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			if(isset($rel1['config_no']) && $rel1['config_no']!=''){
				Db::name('credit_regist')->where(array('regist_id'=>$data['regist_id']))->update(array('qd_id'=>$rel1['config_no']));
				return true;
			}else{
				return false;
			}
			...
			if(isset($rel1['Resp_msg'])  && $rel1['Resp_msg'] == '该用户已入网'){
				return true;
			}
			return false;
			/*if(isset($rel1['Resp_msg'])){
	...
	//C3通道
	public function CompensatoryWithBase(){	
		$data2 = [
			'user_no'=> input('user_no'),
			'rate'=> input('rate'),
			'single_payment'=> input('single_payment'),
			'backUrl'=> input('backUrl'),
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->CompensatoryWithBase($data2);
		
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			if(isset($rel1['url']) && $rel1['url'] != ''){
				return rel(1,$rel1['url']);
				//header("location:".$rel1['url']."");
			}else{
	...
	//C3通道
	public function getUserTaskStatus(){	
		$data2 = [
			'user_no'=> input('user_no'),
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->getUserTaskStatus($data2);
		
				
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			if(isset($rel1['status']) && $rel1['status'] != ''){
				return rel(1,$rel1['status']);
				//header("location:".$rel1['url']."");
			}else{
				if(isset($rel1['Resp_msg']) && $rel1['Resp_msg'] != ''){
					//echo $rel1['Resp_msg'];
	...
	//激活通道
	public function entrycardconfirm(){	
		//$data=$this->_data;
		$data['regist_id'] = input('regist_id');
		$data['yzm'] = input('yzm');
		$data['temp_id'] = input('temp_id');
		$data['entryCardOrderNo'] = input('entryCardOrderNo');
		$credit_regist	=Db::name('credit_regist')->where(array('regist_id'=>$data['regist_id']))->find();
		
		$repayTemp=Db::name('repay_temp')->where('temp_id',$data['temp_id'])->find();	//信用卡预绑定数据
		
		if(!$credit_regist || !$repayTemp){
		   return rel(-1,'数据有误');
		}
		
		if($credit_regist['qdr_id'] !=''){
			$repayLogic =model('common/RepayLogic','Logic');
			$repayLogic->exeProgram($data['regist_id'],$data['temp_id']);
			//Db::name('repay_temp')->where('temp_id',$data['temp_id'])->update(['status'=>2]);
			return rel(1,'还款计划执行成功');
		}
		...
		$data2 = [
			'user_no'=> $credit_regist['mer_id'],
			'smsCode'=> $data['yzm'],
			'entryCardOrderNo'=> $data['entryCardOrderNo'],
			'rate'=> $rate,
			'single_payment'=> $single_payment
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->entrycardconfirm($data2);
		//dump($rel1);
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			if(!isset($rel1['config_no'])){
				return rel(-1,'激活失败');
			}
			Db::name('credit_regist')->where(array('regist_id'=>$data['regist_id']))->update(array('qdr_id'=>$rel1['config_no']));
			$ispaytd = $this->jihuoPay($data);
			if($ispaytd){
				$repayLogic =model('common/RepayLogic','Logic');
				$repayLogic->exeProgram($data['regist_id'],$data['temp_id']);
				//Db::name('repay_temp')->where('temp_id',$data['temp_id'])->update(['status'=>2]);
	...
	//还款任务的执行
	public function compensatorypay(){
		$hour = date('H',time());
		if($hour>=23 || $hour<=1) return false;	
	    /*还款任务执行*/
		$seadata['r.exetime'] = ['elt',time()];
		$seadata['r.status'] = 1;
		$seadata['r.is_post'] = 1;
		$seadata['r.resp_msg'] = NULL;
		$seadata['p.status'] = ['IN','1,2'];
		
		$ghostArr = Db::name('repayList')->alias('r')
		->field('r.*,p.regist_id,p.current,p.city')
		...
		foreach($ghostArr as $ghost){
			//判断是否有未执行的任务
			$prev_drepay= Db::name('repayList')->where(['repay_id'=>$ghost['repay_id'],'id'=>['lt',$ghost['id']],'status'=>1])->order('id asc')->limit(1)->find();
			if($prev_drepay){
				 continue;
			}
			$credit_regist	= Db::name('credit_regist')->where(array('regist_id'=>$ghost['regist_id']))->find();
			
			if($ghost['current'] == 1){
				if($ghost['type'] == 2){
					$rel1=$this->c2insteadpay($credit_regist,$ghost);
				}elseif($ghost['type'] == 1){
					$rel1=$this->c2pay($credit_regist,$ghost);
				}
			}elseif($ghost['current'] == 2){
				if($ghost['type'] == 2){
					$rel1=$this->d3insteadpay($credit_regist,$ghost);
				}elseif($ghost['type'] == 1){
					$rel1=$this->d3pay($credit_regist,$ghost);
				}
			}else{
				if($ghost['type'] == 2){
					$rel1=$this->insteadpay($credit_regist,$ghost);
				}elseif($ghost['type'] == 1){
					$rel1=$this->pay($credit_regist,$ghost);
				}
			}
			
			if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
				Db::name('repayList')->where(['list_id'=>$ghost['list_id']])->update(['resp_msg'=>$rel1['Msg'],'resp'=>json_encode($rel1)]);
			}else{
			    if($ghost['id'] == 1){
				Db::name('repayList')->where(['list_id'=>$ghost['list_id']])->update(['resp_msg'=>$rel1['Msg'],'resp'=>json_encode($rel1),'status'=>3]);
				Db::name('repay_program')->where(['pro_id'=>$ghost['repay_id']])->update(['status'=>5]);


				}else{
				Db::name('repayLog')->insert(array('repay_id'=>$ghost['repay_id'],'list_id'=>$ghost['list_id'],'errormsg'=>json_encode($rel1),'create_time'=>time()));
				
				//Db::name('repayList')->where(['list_id'=>$ghost['list_id']])->update(['order_no'=>order_no()]);
		...
		/*取款任务超时*/
			$seadata2['ctime'] = ['elt',time()-1800];
			$seadata2['status'] = 0;
			$seadata2['keyname'] = 'Th';
			Db::name('get_money')->where($seadata2)->update(['utime'=>time(),'dsc'=>'操作超时','status'=>1]);
		/*取款任务超时*/
		
		/*过期VIP*/
			$seadata3['vip_end_time'] = ['gt',0];
			$seadata3['vip_end_time'] = ['elt',time()];
			$seadata3['group_id'] = 2;
			Db::name('member')->where($seadata3)->update(['group_id'=>1]);
		/*过期VIP*/
	}
	
	
	private function otderpay($credit_regist,$ghostArr){	
		$site = input('server.REQUEST_SCHEME') . '://' . input('server.SERVER_NAME');
		$data = [
			'user_no' => $credit_regist['mer_id'],//商户入网返回的 user_no
			'config_no' =>  $credit_regist['qd_id'],//渠道激活的配置号

			'order_no' => $ghostArr['order_no'],//订单号商户自定义不重复订单号长度不小于16位
			'ypt_order_no' => $ghostArr['r_ypt_order_no'],//同名付返回的 ypt_order_no
			'notifyUrl' =>  $site.'/api/Drepay/notifyUrl',//渠道商接受回调路由,用于异步接受通知
			'price' => $ghostArr['money'],//订单金额 单位元(1 = 人民币 1元)
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->otderpay($data);
		return $rel1;
	}
	
	private function c2pay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
		...
		$data = [
			...
			'price' => $ghostArr['money']/100,//订单金额 单位元(1 = 人民币 1元)
			'order_no' => $ghostArr['order_no'],//订单号商户自定义不重复订单号长度不小于16位
			'notifyUrl' =>  $site.'/api/Drepay/c3notifyUrl',//渠道商接受回调路由,用于异步接受通知
	...
	private function d3pay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
			$drepay_lxr	=Db::name('bank_card')->where(array('status'=>1,'user_id'=>$credit_regist['user_id']))->find();
		...
		$mcc = '';
		if($ghostArr['mcc_p_id']!=''){
			$outapiLogic=model('common/TdapiLogic','Logic');
			$mccres=$outapiLogic->getMacc(['p_id'=>$ghostArr['mcc_p_id']]);
			if(isset($mccres['Code']) && $mccres['Code'] == '10000' && isset($mccres['Resp_code']) && $mccres['Resp_code'] == '40000'){
				if(isset($mccres['data'])){
					$rand_mcc = array_rand($mccres['data'],1);
					if(isset($rand_mcc['mcc'])){
						$mcc = $rand_mcc['mcc'];
					}
				}
			}
		...
		$site = input('server.REQUEST_SCHEME') . '://' . input('server.SERVER_NAME');
		$data = [
			'config_no' =>  $credit_regist['qdrjy_id'],//渠道激活的配置号
			'price' => $ghostArr['money']/100,//订单金额 单位元(1 = 人民币 1元)
			'order_no' => $ghostArr['order_no'],//订单号商户自定义不重复订单号长度不小于16位
			'notifyUrl' =>  $site.'/api/Drepay/c3notifyUrl',//渠道商接受回调路由,用于异步接受通知
			'areaCode' => $areaCode, //地域码 前四位 文档6.4.2获取 city_code 前四位 当值为 0000 时,为全国可用商户
		...
		//$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->d3pay($data);
		return $rel1;
	}
	
	
	private function pay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
			$drepay_lxr	=Db::name('bank_card')->where(array('status'=>1,'user_id'=>$credit_regist['user_id']))->find();
		...
		$data = [
			'user_no' => $credit_regist['mer_id'],//商户入网返回的 user_no
			'config_no' =>  $credit_regist['qdr_id'],//渠道激活的配置号
			'order_no' => $ghostArr['order_no'],//订单号商户自定义不重复订单号长度不小于16位
			'notifyUrl' =>  $site.'/api/Drepay/notifyUrl',//渠道商接受回调路由,用于异步接受通知
			'price' => $ghostArr['money']/100,//订单金额 单位元(1 = 人民币 1元)
			'areaCode' => $areaCode, //地域码 前四位 文档6.4.2获取 city_code 前四位 当值为 0000 时,为全国可用商户
		...
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->pay($data);
		/*if(isset($rel1['Resp_msg']) && $rel1['Resp_msg'] == '该区域不存在门店'){
			$data['areaCode'] = '0000';
			$data['order_no'] = order_no();
			Db::name('drepayList')->where(array('order_no'=>$ghostArr['order_no']))->update(array('order_no'=>$data['order_no']));
			$rel1=$outapiLogic->pay($data);
		}*/
		return $rel1;
	}
	...
	private function c2insteadpay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
			$drepay_lxr	=Db::name('bank_card')->where(array('status'=>1,'user_id'=>$credit_regist['user_id']))->find();
		...
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->c2insteadpay($data);
		return $rel1;
	}
	
	private function d3insteadpay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
			$drepay_lxr	=Db::name('bank_card')->where(array('status'=>1,'user_id'=>$credit_regist['user_id']))->find();
		...
		//查询商户余额
		$balance=$outapiLogic->queryCustomBalance(array('config_no' => $data['config_no'],'channel_no'=>"S16DFCC"));
		if($balance>0){
			$data['price'] = $balance;
		}
		
		$rel1=$outapiLogic->d3insteadpay($data);
		return $rel1;
	}
	
	private function insteadpay($credit_regist,$ghostArr){	
		if($ghostArr['city']!=''){
			$areaCode = substr($ghostArr['city'],0,4);
		}else{
			$drepay_lxr	=Db::name('bank_card')->where(array('status'=>1,'user_id'=>$credit_regist['user_id']))->find();
		...
		$data = [
			...
			'order_no' => $ghostArr['order_no'],//订单号商户自定义不重复订单号长度不小于16位
			'notifyUrl' =>  $site.'/api/Drepay/notifyUrl',//渠道商接受回调路由,用于异步接受通知
			'ypt_order_no' => $ghostArr['ypt_order_no'],
			'price' => $ghostArr['money']/100,//订单金额 单位元(1 = 人民币 1元)
			'areaCode' => $areaCode, //地域码 前四位 文档6.4.2获取 city_code 前四位 当值为 0000 时,为全国可用商户
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		...
		//查询商户余额
		$balance=$outapiLogic->queryCustomBalance(array('config_no' => $data['config_no'],'channel_no'=>"S18HHZTP"));
		if($balance>0){
			$data['price'] = $balance;
		}
		
		$rel1=$outapiLogic->insteadpay($data);
	...
	public function modifyinfo(){
		$data2 = [];
		$data2['user_no'] = input('user_no');
		$data2['bank_branch'] = input('bank_branch');
		$data2['bank_code'] = input('bank_code');
		$data2['validity'] = input('validity');
		$data2['cvv2'] = input('cvv2');
		...
		$data = [
			'user_no' => $data2['user_no'],//新建商户返回的子商户号user_no
			'bank_branch' => $data2['bank_branch'],//储蓄卡:银行卡开户支行名称,信用卡:银行卡开户行名称
			'bank_code' => $data2['bank_code'],//联行号
			'validity' =>  $data2['validity'],//有效期、信用卡必传(mmyy格式)
			'cvv2' =>  $data2['cvv2'],//安全码、信用卡必传
		];
		
		//file_put_contents('log/Tdlog/Tdlog.txt',json_encode($data),FILE_APPEND);
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->modifyinfo($data);
			...
			if(isset($rel1['confirm_status'])){
				if($rel1['confirm_status']==2){
					if(isset($rel1['config_no']) && $rel1['config_no']!=''){
						...
						$repayLogic =model('common/RepayLogic','Logic');
						$repayLogic->exeProgram($rel1['regist_id'],$rel1['temp_id']);
						echo 'success';
						exit;
					}
				}
	...
	public function d3notifyUrl(){
		...
		if(isset($rel1['Code']) && $rel1['Code'] == '10000'){
			if(isset($rel1['confirm_status'])){
				if($rel1['confirm_status']==2){
					if(isset($rel1['config_no']) && $rel1['config_no']!=''){
						Db::name('credit_regist')->where(array('regist_id'=>$rel1['regist_id']))->update(array('qdrjy_id'=>$rel1['config_no']));
						$repayLogic =model('common/RepayLogic','Logic');
						$repayLogic->exeProgram($rel1['regist_id'],$rel1['temp_id']);
						echo 'success';
						exit;
	...
	public function notifyUrl(){
		...
		if(isset($rel1['order_no']) && $rel1['order_no'] != ''){	
			$ghostArr=Db::name('repayList')->where(['order_no'=>$rel1['order_no']])->find();
			if($ghostArr && ($ghostArr['status']==1 || $ghostArr['status']==3)){
				if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
					Db::name('repayList')->where(['order_no'=>$rel1['order_no']])->update(['order_num'=>$rel1['ypt_order_no'],'resp_msg'=>$rel1['Resp_msg'],'resp'=>json_encode($rel1),'status'=>2,'cutime'=>time()]);
					
					//如果是消费，则更新还款
					if($ghostArr['type'] == 1){
						   $min_list = Db::name('repayList')->where(['repay_id'=>$ghostArr['repay_id'],'status'=>1,'type'=>2])->order('id asc')->limit(1)->find();	
							
							if($min_list['ypt_order_no'] == ''){
							  $ypt_order_no = $rel1['ypt_order_no'];
							}else{
								  $ypt_order_no = $min_list['ypt_order_no'].','.$rel1['ypt_order_no'];
							}
							Db::name('repayList')->where(['list_id'=>$min_list['list_id']])->update(['ypt_order_no'=>$ypt_order_no]);	
					}
					
					
					//获取下一条
					$next_drepay= Db::name('repayList')->where(['repay_id'=>$ghostArr['repay_id'],'id'=>['gt',$ghostArr['id']],'status'=>1])->order('id asc')->limit(1)->find();
					
					if(!$next_drepay){
						Db::name('repay_program')->where(['pro_id'=>$ghostArr['repay_id']])->update(['status'=>3]);
						//发送短信
						$phone = Db::name('member')->where(['id'=>$ghostArr['user_id']])->value("account");
						$content = "【".config('siteName')."】您的在平台执行的还款计划已经完成，请前往APP查看详情，欢迎再次使用。";
						$this->sendSmsByzk($phone,$content);
					}else{
						Db::name('repay_program')->where(['pro_id'=>$ghostArr['repay_id']])->update(['status'=>2]);
						Db::name('repayList')->where(['list_id'=>$next_drepay['list_id']])->update(['is_post'=>1]);
					}
					
                    //更新积分
					insertCreditValLog(['order_no'=>$rel1['order_no'],'user_id'=>$ghostArr['user_id'],'form_userid'=>0,'type'=>2,'val'=>round(($ghostArr['d_money']+$ghostArr['s_money'])/100,0)]);
					...
					Db::name('repayList')->where(['order_no'=>$rel1['order_no']])->update(['resp_msg'=>$msg,'resp'=>json_encode($rel1),'status'=>3,'cutime'=>time()]);
					
					Db::name('repay_program')->where(['pro_id'=>$ghostArr['repay_id']])->update(['status'=>5]);
					$phone = Db::name('member')->where(['id'=>$ghostArr['user_id']])->value("account");
					if($ghostArr['type'] == 2){
						$content = "【".config('siteName')."】您在平台进行的信用卡还款，因还款失败，导致还款任务终止，请前往APP查看详细信息。";
					}else{
						$content = "【".config('siteName')."】您在平台进行的信用卡还款，因扣款失败，导致还款任务终止，请前往APP查看详细信息。";
					}
					$this->sendSmsByzk($phone,$content);
					return ['Code'=>10000];
				}	
	...
	public function entrycard(){
		$data2=$this->_data;
		$data['regist_id'] = $data2["registId"];
		$credit_regist	=Db::name('credit_regist')->where(array('regist_id'=>$data['regist_id']))->find();
		$data = [
			'user_no'=> $credit_regist['mer_id'],
		];
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->entrycard($data);
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			
			if( isset($rel1['Resp_msg']) && $rel1['Resp_msg']=='签约成功，重复签约'){
				$phone = Db::name('member')->where(['id'=>$credit_regist['user_id']])->value("account");
				$content = "【".config('siteName')."】您已经成功签约，短信验证码请直接输入12345。";
				$this->sendSmsByzk($phone,$content);
			}
			//Db::name('credit_regist')->where(array('regist_id'=>$data['regist_id']))->update(array('qd_id'=>$rel1['config_no']));
			return rel(1,'发送成功',$rel1['ypt_order_no']);
			
			//return rel(1,'发送成功',$rel1['entryCardOrderNo']);
		}else{
			if(isset($rel1['Resp_msg'])){
				return rel(-1,$rel1['Resp_msg']);
			}else{
	...
	public function getwallet(){
		...
		$outapiLogic=model('common/TdapiLogic','Logic');
		$rel1=$outapiLogic->getwallet();
		dump($rel1);
	}

	

   private function get_shenfen($id){
        //截取前两位数
        $index = substr($id,0,2);
        $area = array(
            11 => "北京",
  ...
  private function gethometownByIdcard($num){
  	$hometown_arr=array(
			'110000' => '北京市',
			'110100' => '北京市市辖区',
			'110101' => '北京市东城区',
			...
			'150430' => '内蒙古自治区赤峰市敖汉旗',
			'152100' => '内蒙古自治区呼伦贝尔市',
			'152101' => '内蒙古自治区呼伦贝尔市海拉尔区',
			'152102' => '内蒙古自治区呼伦贝尔市满洲里市',
			'152103' => '内蒙古自治区呼伦贝尔市扎兰屯市',
			'152104' => '内蒙古自治区呼伦贝尔市牙克石市',
			'152105' => '内蒙古自治区呼伦贝尔市根河市',
			'152106' => '内蒙古自治区呼伦贝尔市额尔古纳市',
			'152122' => '内蒙古自治区呼伦贝尔市阿荣旗',
			'152123' => '内蒙古自治区呼伦贝尔市莫力达瓦达斡尔族自治旗',
			'152127' => '内蒙古自治区呼伦贝尔市鄂伦春自治旗',
			'152128' => '内蒙古自治区呼伦贝尔市鄂温克族自治旗',
			'152129' => '内蒙古自治区呼伦贝尔市新巴尔虎右旗',
			'152130' => '内蒙古自治区呼伦贝尔市新巴尔虎左旗',
			'152131' => '内蒙古自治区呼伦贝尔市陈巴尔虎旗',
			'152200' => '内蒙古自治区兴安盟',
		...
		if(@$hometown_arr[$num])return $hometown_arr[$num];
		else return '0';
  }
