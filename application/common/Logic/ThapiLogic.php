 ...
 * 星洁第三方支付接口的逻辑对象
 */
class ThapiLogic extends Model{
	...
	public function authent($data){
		$user = Db::name('member')->where(['id' => $data['user_id']])->find();
		$bank_card = Db::name('bank_card')->where(['type'=>1,'status'=>1,'user_id' => $data['user_id']])->find();
		$reqData = [
			...
			'paymer_name' => $bank_card['name'],//持卡人姓名
			'paymer_idcard' => $bank_card['idcard'],//持卡人身份证
			'paymer_bank_no' => $bank_card['bank_code'],//持卡人银行卡号
			'paymer_phone' => $bank_card['phone'],//持卡人预留手机号
		];
		...
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			...
			$data2 = [
				'auth_order_no' => $rel1['authent_no'],//鉴权订单号(鉴权接口返回)
				'user_name' => $user['realname'],//持卡人姓名
				'id_no' => $user['card'],//身份证号
				'card_no' => $bank_card['bank_code'],//银行卡号
				'phone' => $bank_card['phone'],//银行预留手机号
				'bank_name' => $bank_card['bank_name'],//银行卡开户行名称
				'bank_branch' => $bank_card['bank_child'],//储蓄卡:银行卡开户支行名称,信用卡:银行卡开户行名称
				'bank_code' => $bank_card['bank_id'],//联行号
				'bank_coding' => $bank_card['bank_no'],//银行缩写
				'province' => $shenfen,//省 (身份证)
				'city' => $area,//城市(身份证)
				'county' => $area,//区、县(身份证)
				'address' => $area,//详细地址
				'validity' => '',//有效期、信用卡必传(mmyy格式)
				'cvv2' => '',//安全码、信用卡必传
				'bank_type' => 1,//银行卡类型 1表示储蓄卡6表示信用
			];
			$rel2 = $this->enternet($data2);
	...
	//鉴权提现
	public function authentCash($data){
		$user = Db::name('member')->where(['id' => $data['user_id']])->find();
		...
		$reqData = [
			...
			'channel_no' => 'S7SJBZP',//固定值 ‘S7SJBZP’
			'business_no' => 'authent_channel',//固定值 ‘authent_channel’
			'paymer_name' => $agent['bank_real_name'],//持卡人姓名
			'paymer_idcard' => $user['card'],//持卡人身份证
			'paymer_bank_no' => $agent['bank_code'],//持卡人银行卡号
			'paymer_phone' => $user['account'],//持卡人预留手机号
		];
		$url = $this->produce_url.'/api/Authent/authent';
		$reqData['sign'] = $this->MakeSign($reqData);
		//dump($reqData);die;
		...
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			//新建个体商户
			$bank_card = Db::name('bank_info')->where(['bank_name' => $agent['bank_name']])->find();
			$shenfen = $this->get_shenfen($user['card']);
			$area= $this->gethometownByIdcard(substr($user['card'],0,6));
			$data2 = [
				'auth_order_no' => $rel1['authent_no'],//鉴权订单号(鉴权接口返回)
				'user_name' => $user['realname'],//持卡人姓名
				'id_no' => $user['card'],//身份证号
				'card_no' => $agent['bank_code'],//银行卡号
				'phone' => $user['account'],//银行预留手机号
				'bank_name' => $bank_card['bank_name'],//银行卡开户行名称
				'bank_branch' => $bank_card['bank_name'],//储蓄卡:银行卡开户支行名称,信用卡:银行卡开户行名称
				'bank_code' => $bank_card['lineno'],//联行号
				'bank_coding' => $bank_card['acronym'],//银行缩写
				'province' => $shenfen,//省 (身份证)
				'city' => $area,//城市(身份证)
				'county' => $area,//区、县(身份证)
				'address' => $area,//详细地址
		...
		return $rel1;
			...
			//鉴权
	public function authentcredit($data,$creditInfo){
		$user = Db::name('member')->where(['id' => $data['user_id']])->find();
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号
			'channel_no' => 'S7SJBZP',//固定值 ‘S7SJBZP’
			'business_no' => 'authent_channel',//固定值 ‘authent_channel’
			'paymer_name' => $creditInfo['name'],//持卡人姓名
			'paymer_idcard' => $creditInfo['idcard'],//持卡人身份证
			'paymer_bank_no' => $creditInfo['credit_code'],//持卡人银行卡号
			'paymer_phone' => $creditInfo['phone'],//持卡人预留手机号
		];
		$url = $this->produce_url.'/api/Authent/authent';
		$reqData['sign'] = $this->MakeSign($reqData);
		//dump($reqData);die;
		//writeF('Tdlog.txt',json_encode($reqData));
		$rel = Curl::http_curl($url,$reqData);
		$rel1 = json_decode($rel,true);
		writeF('Tdlog.txt',json_encode($rel1));

		//$rel1['authent_no'] = 'TZ20180823190327AC4175E3';
		//$rel1['Code'] = '10000';
		//$rel1['Resp_code'] = '40000';
		//如果鉴权成功，去建商户
		if(isset($rel1['Code']) && $rel1['Code'] == '10000' && isset($rel1['Resp_code']) && $rel1['Resp_code'] == '40000'){
			//新建个体商户
			$shenfen = $this->get_shenfen($user['card']);
			...
			$data2 = [
				...
				'user_name' => $user['realname'],//持卡人姓名
				'id_no' => $user['card'],//身份证号
				'card_no' => $creditInfo['credit_code'],//银行卡号
				'phone' => $creditInfo['phone'],//银行预留手机号
				'bank_name' => $creditInfo['bank_name'],//银行卡开户行名称
				'bank_branch' => $creditInfo['bank_name'],//储蓄卡:银行卡开户支行名称,信用卡:银行卡开户行名称
				'bank_code' => $creditInfo['bank_id'],//联行号
				'bank_coding' => $creditInfo['bank_no'],//银行缩写
				'province' => $shenfen,//省 (身份证)
				'city' => $area,//城市(身份证)
				'county' => $area,//区、县(身份证)
				'address' => $area,//详细地址
				'validity' => $creditInfo['exp_date'],//有效期、信用卡必传(mmyy格式)
				'cvv2' => $creditInfo['safe_code'],//安全码、信用卡必
	...
	//进件
	public function withdrawentry($data){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号
			'user_no' => $data['merid'],//用户商户号（ 商户入网返回的 user_no）
			'business_no' => 'pay_channel',//业务标识（固定值：pay_channel）
			'channel_no' => 'ARKYUN',//通道标识
			'rate' => $data['rate']/10,//用户费率（ 渠道商给终端用户费率 0.5 是指 0.5%）
			'single_payment' => $data['dfee']/100,//用户代付费（渠道商给终端用户单笔代付费 1 是指 1 元/笔）
			'rest' => '',//其他参数
		];
		$url = $this->produce_url.'/api/Withdraw/entryCard';
		$reqData['sign'] = $this->MakeSign($reqData);
		$rel = Curl::http_curl($url,$reqData);
		$rel = json_decode($rel,true);
		return $rel;		
	...
	//费率修改
	public function withdrawchangerate(){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号
			'user_no' => $data['mchId'],//用户商户号（ 商户入网返回的 user_no）
			'business_no' => 'pay_channel',//业务标识（固定值：pay_channel）
			'channel_no' => 'ARKYUN',//通道标识
			'rate' => $data['rate']/10,//用户费率（ 渠道商给终端用户费率 0.5 是指 0.5%）
			'single_payment' => $data['dfee']/100,//用户代付费（渠道商给终端用户单笔代付费 1 是指 1 元/笔）
			'config_no' => $data['merkey'],//银行卡进件后返回的商户号
			'rest' => '',//其他参数
 		];
 		$url = $this->produce_url.'/api/Withdraw/changeCardRate';
		$reqData['sign'] = $this->MakeSign($reqData);
		$rel = Curl::http_curl($url,$reqData);
		$rel = json_decode($rel,true);
		return $rel;
	...
	//新无卡支付
	public function polypay($data){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号)
			'business_no' => 'pay_channel',//业务标识 （固定值：pay_channel）
			'channel_no' => 'S14HHZTP',//通道标识
			'bank_card_no' => $data['mchId'],//子商户号（银行卡在特定通道进件的子商户号）
			'credit_card_no' => $data['bank_user_no'],//用户商户号（信用卡入网返回的商户号）
			'price' => $data['totalFee']/100,//支付金额 单位元(1 = 人民币 1元)
			'rate' => $data['rate']/10,
			'single_payment' => $data['d0fee']/100,
			'order_no' => $data['agentOrderNo'],//商户订单号 （每笔订单号必须唯一）
			'web_notify_url' => $data['returnUrl'],//支付成功跳转的链接
			'notifyUrl' => $data['notifyUrl'],//支付回调地址
		];
		
		$url = $this->produce_url.'/api/Poly/polypay';
		$reqData['sign'] = $this->MakeSign($reqData);
		$rel = Curl::http_curl($url,$reqData);
		//writeF('Thlog.txt',$rel.'5');
		$rel = json_decode($rel,true);
		return $rel;
	...
	//聚合落地取款
	public function fallPay($data){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号)
			'business_no' => 'pay_channel',//业务标识 （固定值：pay_channel）
			'channel_no' => 'S23HHZTP',//通道标识
			'bank_card_no' => $data['mchId'],//子商户号（银行卡在特定通道进件的子商户号）
			'credit_card_no' => $data['bank_user_no'],//用户商户号（信用卡入网返回的商户号）
			'price' => $data['totalFee']/100,//支付金额 单位元(1 = 人民币 1元)
			'rate' => $data['rate']/10,
		...
		$url = $this->produce_url.'/api/Fall/fallPay';
		$reqData['sign'] = $this->MakeSign($reqData);
		$rel = Curl::http_curl($url,$reqData);
		//writeF('Thlog.txt',$rel.'5');
		$rel = json_decode($rel,true);
		...
		//支付
	public function withdrawpay($data){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号
			'user_no' => $data['mchId'],//用户商户号(银行卡入网返回的商户号)
			'business_no' => 'pay_channel',//业务标识 （固定值：pay_channel）
			'channel_no' => 'ARKYUN',//通道标识
			'config_no' => $data['merkey'],//子商户号（银行卡在特定通道进件的子商户号）
			'bank_user_no' => $data['bank_user_no'],//用户商户号（信用卡入网返回的商户号）
			'price' => $data['totalFee']/100,//支付金额 单位元(1 = 人民币 1元)
			'order_no' => $data['agentOrderNo'],//商户订单号 （每笔订单号必须唯一）
			'returnUrl' => $data['returnUrl'],//支付成功跳转的链接
			'notifyUrl' => $data['notifyUrl'],//支付回调地址
		];
		$url = $this->produce_url.'/api/Withdraw/Pay';
		writeF('Thlog.txt',json_encode($reqData).'1');
		$reqData['sign'] = $this->MakeSign($reqData);
		$rel = Curl::http_curl($url,$reqData);
		$rel = json_decode($rel,true);
		return $rel;		
	...
	public function work($fun,$data,$isday){
		if($fun == "pay"){
			//检查是否有通道
			//
			$block_id = 9;
			$aisle=Db::name('payAisle')->where('block_id',$block_id)->find();
			$registInfo=Db::name('creditRegist')->where(['credit_code'=>$data['bankCode'],'block_id'=>['in',[6,$block_id]],'status'=>1])->order("regist_id desc")->find();
		    if(!$registInfo){
		    	   $creditInfo=Db::name('creditCard')->where(['credit_code'=>$data['bankCode'],'status'=>1])->find();
					if(!$creditInfo){
						return ['code'=>-1,'msg'=>'该信用卡无效','result'=>''];
				    }
		    	   //$data['year']=substr($y, -2, 2);
					...
					$result = $this->authentcredit($data,$creditInfo);
					writeF('Thlog.txt',json_encode($result).'2');
					if(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && ($result['Resp_code'] == '40000' || $result['Resp_code'] == '40007')){

						$data['bank_user_no'] = $result['user_no'];
						//$res2  = $this->withdrawentry($data);
						//dump($res2);
						
						//if(isset($res2['Code']) && $res2['Code'] == '10000' && isset($res2['Resp_code']) && $res2['Resp_code'] == '40000'){
						Db::name('creditRegist')->where('regist_id',$data['regist_id'])->update(['utime'=>time(),'status'=>1,'mer_id'=>$result['user_no']]);
					   // }
						//return ['code'=>1,'msg'=>$result['Resp_msg'],'data'=>$result['user_no']];
					}else{
						if(isset($result['Resp_msg']) && $result['Resp_msg']!=''){
							return ['code'=>-1,'msg'=>$result['Resp_msg'],'result'=>''];	
						}elseif(isset($result['Msg']) && $result['Msg']!=''){
							return ['code'=>-1,'msg'=>$result['Msg'],'result'=>''];	
						}else{
							return ['code'=>-1,'msg'=>'信用卡进件失败','result'=>''];	
						}
					}
		    }else{
		    	$data['bank_user_no'] = $registInfo['mer_id'];
		    }

			if(isset($data['is_ge']) && $data['is_ge']==1){
				$result = $this->fallPay($data);
			}else{
				$result = $this->polypay($data);
			}
			writeF('Thlog.txt',json_encode($result).'3');
			if(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40000'){
					...
					return ['code'=>1,'msg'=>$result['Resp_msg'],'result'=>$resg];
				
			}elseif($result!=''){
				if(isset($result['Resp_msg'])){
					return ['code'=>-1,'msg'=>$result['Resp_msg']];
				}else{
					return ['code'=>-1,'msg'=>$result['Msg']];
				}
				
			}
			return ['code'=>-1,'msg'=>'通道请求太频繁，请稍后再试','result'=>''];
		}elseif($fun ==  "regist"){

			$result = $this->authent($data);
			writeF('Thlog.txt',json_encode($result).'4');
			if(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40000'){

				/*$data['merid'] = $result['user_no'];
				$res2  = $this->withdrawentry($data);
				//dump($res2);
				
				if(isset($res2['Code']) && $res2['Code'] == '10000' && isset($res2['Resp_code']) && $res2['Resp_code'] == '40000'){
						Db::name('bankRegist')->where('regist_id',$data['regist_id'])->update(['merkey'=>$res2['config_no']]);
			    }*/
				return ['code'=>1,'msg'=>$result['Resp_msg'],'data'=>$result['user_no']];
			}elseif(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40007'){
				//已存在
				/*$data['merid'] = $result['user_no'];
				$res2  = $this->withdrawentry($data);
				//dump($res2);
			...
			//dump($result);
			return ['code'=>-1,'msg'=>'通道请求失败','result'=>''];
		}elseif($fun ==  "registCash"){

			$result = $this->authentCash($data);
			writeF('Thlog.txt',json_encode($result).'9');
			if(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40000'){
				return ['code'=>1,'msg'=>$result['Resp_msg'],'data'=>$result['user_no']];
			}elseif(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40007'){
				//已存在
			...
			/*$result = $this->withdrawchangerate($data);
			if(isset($result['Code']) && $result['Code'] == '10000' && isset($result['Resp_code']) && $result['Resp_code'] == '40000'){
				
				Db::name('bankRegist')->where('regist_id',$data['regist_id'])->update(['merkey'=>$result['config_no']]);
				 return ['code'=>1,'msg'=>$result['Resp_msg'],'data'=>$data['mchId']];
			}*/
			return ['code'=>1,'msg'=>'不修改','result'=>''];
		}elseif($fun ==  "see"){
               
          return ['code'=>-1,'msg'=>'通道请求失败','result'=>''];  
		}else{
			return ['code'=>-1,'msg'=>'未知请求方法','result'=>''];
		}
	...
	//激活代付通道
	public function bankEntryCard($data){
		$reqData = $data;
		$reqData['merchant_no'] = $this->seller_no;
		$reqData['channel_no'] = 'S21HHZTPDF';
		$reqData['business_no'] = 'agency_pay_channel';
		$url = $this->produce_url.'/api/Other/bankEntryCard';
		$reqData['sign'] = $this->MakeSign($reqData);
		//dump($reqData);die;
		$rel = Curl::http_curl($url,$reqData);
		$rel = json_decode($rel,true);
	...
	public function insteadpay($data){
		$reqData = [
			'merchant_no' => $this->seller_no,//渠道商商户号YPT分配的渠道商户号
			'channel_no' => 'S21HHZTPDF',//固定值 ‘S8HHZTP’
			'business_no' => 'agency_pay_channel',//固定值 ‘back_channel’
